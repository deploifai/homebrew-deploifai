...
def generate_new_formula(placeholder_formula, resource_stanzas, new_tag, new_version):
    ...
    for line in placeholder_formula:
        ...
        if "NEW_TAG" in line:
            line = line.replace("NEW_TAG", new_tag)
        elif "NEW_VERSION" in line:
            line = line.replace("NEW_VERSION", new_version)
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Please provide the new tag and version as the first two arguments.")
        sys.exit(1)

    # get the new tag from the first argument
    new_tag = sys.argv[1]
    print(f"New tag: {new_tag}")

    # get the new version from the second argument
    ...
    new_formula = generate_new_formula(
        placeholder_formula_file.readlines(), resource_stanzas, new_tag, new_version
    )
